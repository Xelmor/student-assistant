...
class AppSmokeTests(unittest.TestCase):
    ...
    def test_base_template_uses_local_shell_assets(self):
        ...
        self.assertIn('/static/css/style.css?v=20260922-graphite', text)
        self.assertIn('/static/js/base.js?v=20260611-motion-v1', text)
        self.assertIn('/static/js/user-preferences.js?v=20260922-graphite', text)
        ...
        self.assertIn('/static/js/pwa.js?v=20260612-telegram-v1', text)
    ...
    def test_pwa_install_prompt_uses_graphite_surface(self):
        ...
        self.assertIn('body .install-sheet-card', theme)
        self.assertIn('background: var(--bg-secondary)', theme)
        self.assertIn('backdrop-filter: blur(4px)', theme)
    ...
    def test_dashboard_onboarding_uses_persistent_dark_checklist(self):
        ...
        self.assertIn('linear-gradient(145deg, var(--surface), var(--bg-secondary))', styles)
    ...
    def test_first_run_onboarding_chat_uses_scripted_conversation(self):
        ...
        self.assertIn('const accentChoices = [', script)
        self.assertIn('createAccentPalette', script)
        self.assertIn('onboarding-chat__color-grid', styles)
        self.assertIn('grid-template-columns: repeat(3, minmax(0, 1fr))', styles)
        for accent in ('purple', 'blue', 'cyan', 'green', 'orange', 'pink'):
            self.assertIn(f"value: '{accent}'", script)
            self.assertIn(f"'{accent}'", route_text)
            self.assertIn(f"('{accent}',", profile_text)
